...
import type { ManagementActionItem } from '@/modules/management/types'
import { cn } from '@/lib/utils'
import {
  AlertTriangle, Baby, Droplets, CheckCircle2, ChevronRight,
  Camera, Bell, Heart, Stethoscope,
} from 'lucide-react'
...
function ActionCard({ it }: { it: ManagementActionItem }) {
  return (
    <Link
      href={it.href}
      className="flex items-start gap-3 py-3 first:pt-0 last:pb-0 hover:bg-muted/30 -mx-4 px-4 rounded-lg transition-colors"
    >
    ...
    <div className="rounded-xl border border-border bg-card p-4">
      ...
      {items.length === 0 ? (
        ...
        <div className="divide-y divide-border">
          {shown.map((it) => <ActionCard key={it.id} it={it} />)}
          {rest > 0 && (
    ...
    <div className="space-y-4 pb-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
        ...
        <SummaryCard
          ...
          value={summary.closeToCalving}
          color="text-violet-500"
        />
        <SummaryCard
          label="A secar"
          value={summary.dueToDryOff}
          color="text-amber-500"
        />
        <SummaryCard
          label="Bezerros incompletos"
          value={summary.incompleteCalves}
